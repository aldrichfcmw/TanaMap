          ...
          <tbody class="table-border-bottom-0">
              @foreach ($data as $key => $d)
                <tr>
                  ...
                  <td>{{ $d -> longitude }}</td>
                  <td>{{ $d -> image  }}</td>
                  {{-- <td><img src="data:image/png;base64,{{ $d->base64_image }}" alt="{{ $d->image }}"> --}}
                  </td>
                  <td>
                      <div class="dropdown">
                        <button type="button" class="btn p-0 dropdown-toggle hide-arrow" data-bs-toggle="dropdown"><i class="bx bx-dots-vertical-rounded"></i></button>
                        <div class="dropdown-menu">
                          ...
                          <button class="dropdown-item" onclick="confirmDelete('{{ $d->id }}','{{ $d->disease_name }}')"><i class="bx bx-trash me-1"></i> Delete</button> 
                        ...
                        <div class="modal-dialog modal-dialog-centered" role="document">
                          <div class="modal-content">
                            ...
                            <div class="modal-body">
                              <h5>Apakah Anda yakin ingin menghapus pengguna <span id="user">{{ $d->disease_name }}</span>?</h5>
...
<div class="modal fade" id="deleteModalAll" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <form action="{{route('data.delete',['type'=>'disease','uname'=> 'all'])}}" method="post">
          @csrf
          @method('DELETE')
          <button type="button" class="btn btn-label-secondary" data-bs-dismiss="modal">Batal</button>
          <button type="submit" class="btn btn-warning">Ya, Hapus</button>
